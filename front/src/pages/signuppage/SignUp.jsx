            ...
            <div className="flex flex-col items-center w-[50rem] h-[50rem] mt-[3rem] shadow-xss rounded-[2rem] pb-4 bg-[#F6F8FA]">
                <section className="flex flex-col border-b-1 border-[#ccc] h-[25rem]">
                    ...
                    <div className='flex justify-center items-center'>
                        <div className="w-[90%] h-[240px] mt-[10px] rounded-2xl border-1 border-gray-500 text-lg font-semibold p-[20px] overflow-scroll">
                            <TermsOfUse />
                        </div>
                    </div>
                    <div className="flex justify-center items-center h-[90px] gap-[100px]">
                        <div className='font-semibold'>
                            <input type='radio' id='agree' name='agree' value='true' onClick={event => setAgreement(event.target.value)} className='cursor-pointer'></input>
                            <label htmlFor='agree' className='pl-[8px] mr-[20px] cursor-pointer'>동의합니다.</label>    
                            <input type='radio' id='disagree' name='agree' value='false' onClick={event => setAgreement(event.target.value)} className='cursor-pointer'></input>
                            <label htmlFor='disagree' className='pl-[8px] cursor-pointer'>동의하지 않습니다.</label>
                        </div>
                        <div>
                ...
                <section className="flex flex-col w-[600px] mt-[15px]">
                    ...
                    <div className='flex justify-between mb-[38px]'>
                        <div>
                            ...
                            <input type="email" name="email" value={email} onChange={event => setEmail(event.target.value)} placeholder="example@example.com" className=" w-[250px] h-[30px] bg-white text-[#6C6C6C] border-2 border-[#D7D7D7] rounded-[10px] pl-[14px]">
                            </input>
                            {errors.email && <div className='text-red-500 mb-[-25px] w-[280px] text-[14px]'>{errors.email}</div>}
                        </div>
                        <div>
                            <label htmlFor="nationality" className="font-medium text-[16px] block mb-[7px]">
                                국적 / Nationality
                            </label>
                            <select name="nationality" value={nationality} onChange={event => setNationality(event.target.value)} className="w-[250px] h-[30px] bg-white text-[#6C6C6C] border-2 border-[#D7D7D7] rounded-[10px] pl-[14px]">
                    ...
                    <div className='flex justify-between mb-[38px]'>
                        <div>
                            <label htmlFor="password" className="font-medium text-[16px] block mb-[7px]">
                                비밀번호 / Password
                            </label>
                            <input type="password" name="password" placeholder="••••••••" value={password} onChange={event => setPassword(event.target.value)} className="w-[250px] h-[30px] bg-white text-[#6C6C6C] border-2 border-[#D7D7D7] rounded-[10px] pl-[14px]">
                            </input>
                            {errors.password && <p className='text-red-500 mb-[-42px] w-[280px] text-[14px]'>{errors.password}</p>}
                        </div>
                        <div>
                            <label htmlFor="date" className="font-medium text-[16px] block mb-[7px]">
                                생년월일 / Date of birth
                            </label>
                            <input type="date" name="date" value={birthDate} onChange={event => setBirthDate(event.target.value)} className="w-[250px] h-[30px] bg-white text-[#6C6C6C] border-2 border-[#D7D7D7] rounded-[10px] pl-[14px]">
                            </input>
                            {errors.birthDate && <p className='text-red-500 mb-[-25px] text-[14px]'>{errors.birthDate}</p>}
                        </div>
                    </div>
                    <div className='flex justify-between mb-[20px]'>
                        <div>
                            <label htmlFor="password-confirm" className="font-medium text-[16px] block mb-[7px]">
                                비밀번호 확인 / Password confirm
                            </label>
                            <input type="password" name="password-confirm" placeholder="••••••••" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} className="w-[250px] h-[30px] bg-white text-[#6C6C6C] border-2 border-[#D7D7D7] rounded-[10px] pl-[14px]">
                            </input>
                            {errors.confirmPassword && <p className='text-red-500 mb-[-25px] w-[280px] text-[14px]'>{errors.confirmPassword}</p>}
                        </div>
                        <div>
                            <label htmlFor="authority" className="font-medium text-[16px] block mb-[7px]">
                                발행관청 / Authority
                            </label>
                            <input type="authority" name="authority" value='MarbleUs' className="w-[250px] h-[30px] bg-white text-[#6C6C6C] border-2 border-[#D7D7D7] rounded-[10px] pl-[14px]">
                            </input>
                        </div>
                    </div>
                </section>
