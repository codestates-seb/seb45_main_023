...
export default function PostDetail({ profile_pic }) {
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_SERVER_URL}/missions/${blogData.member.id}/${cityId}`,
          {
            headers: {
              'Content-Type': 'application/json',
              'ngrok-skip-browser-warning': '69420',
    ...
    <div className="PostContainer">
      {isEditing ? (
        <div>
          ...
          <div>
            <Editor body={editedBody} imageArr={editedImage} setImageArr={setImageArr} setBody={setEditedBody} />
